...
elseif( $step==3 )
{
	$fnom = 'import_'.$_SESSION['BASE'].'_professeurs_directeurs.txt';
	...
	foreach($DB_TAB as $DB_ROW)
	{
		...
		$tab_professeurs_directeurs_base['reference'][$DB_ROW['user_id']]  = $DB_ROW['user_reference'];
		$tab_professeurs_directeurs_base['profil'][$DB_ROW['user_id']]     = $DB_ROW['user_profil'];
		$tab_professeurs_directeurs_base['nom'][$DB_ROW['user_id']]        = $DB_ROW['user_nom'];
		$tab_professeurs_directeurs_base['prenom'][$DB_ROW['user_id']]     = $DB_ROW['user_prenom'];
		$tab_professeurs_directeurs_base['statut'][$DB_ROW['user_id']]     = $DB_ROW['user_statut'];
	}
...
elseif( $step==4 )
{
	$fnom = 'import_'.$_SESSION['BASE'].'_traitement_professeur_directeur.txt';
	if(!file_exists($dossier_import.$fnom))
	{
	...
	foreach($DB_TAB as $DB_ROW)
	{
		...
		$lignes .= '<tr'.$class.'><td>'.html($DB_ROW['user_num_sconet']).'</td><td>'.html($DB_ROW['user_reference']).'</td><td>'.html($DB_ROW['user_profil']).'</td><td>'.html($DB_ROW['user_nom']).'</td><td>'.html($DB_ROW['user_prenom']).'</td><td'.$class.'>'.html($DB_ROW['user_login']).'</td>'.$td_password.'<td>'.$tab_statut[$DB_ROW['user_statut']].'</td></tr>'."\r\n";
	}
	...
	if($nb_add)
	{
		...
		$fnom = 'identifiants_'.$_SESSION['BASE'].'_profs_'.time();
		$zip = new ZipArchive();
