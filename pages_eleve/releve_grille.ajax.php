...
if( $matiere_id && $niveau_id && $matiere_nom && $niveau_nom && $remplissage && $orientation && $marge_min && $couleur && $cases_nb && $cases_largeur && $cases_hauteur )
{
	...
	{
		$DB_TAB = DB_STRUCTURE_lister_result_eleve($_SESSION['USER_ID'] , $liste_item , $date_debut=false , $date_fin=false);
		foreach($DB_TAB as $DB_ROW)
		{
			$tab_eval[$_SESSION['USER_ID']][$DB_ROW['item_id']][] = array('note'=>$DB_ROW['note'],'date'=>$DB_ROW['date'],'info'=>$DB_ROW['info']);
		}
	}
	...
	foreach($tab_eleve as $eleve_id)
	{
		...
		{
			$eval_nb = (isset($tab_eval[$_SESSION['USER_ID']][$item_id])) ? count($tab_eval[$_SESSION['USER_ID']][$item_id]) : 0;
			if($eval_nb>$cases_nb)
			{
				$tab_eval[$_SESSION['USER_ID']][$item_id] = array_slice($tab_eval[$_SESSION['USER_ID']][$item_id],$eval_nb-$cases_nb);
			}
		}
	}
