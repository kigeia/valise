...
// Test si affichage de formulaires spéciaux
$demande_gestion   = ( (isset($_GET['gestion']))   && (HEBERGEUR_INSTALLATION=='multi-structures') ) ? 1 : 0 ;
$demande_webmestre = ( (isset($_GET['webmestre'])) && (HEBERGEUR_INSTALLATION=='multi-structures') ) ? 1 : 0 ;
?>
...
{
	$chemin_fichier = './__tmp/dump-base/db_backup.'.SACOCHE_STRUCTURE_BD_NAME.'.'.date("Y-m-d").'.sql.gz';
	if(!is_file($chemin_fichier))
	{
		...
		echo'<img alt="" src="./__DBsvg/backupDB.php?nohtml=1&amp;onlyDB='.SACOCHE_STRUCTURE_BD_NAME.'&amp;StartBackup=standard&amp;mailto=1" />';
	}
}
*/
?>
...
<!-- <div> -->
	...
	// Plus possible depuis qu'il y a 1 bahut / base
	?>
<!-- </div> -->
...
<ul class="puce">
	...
	if(is_dir('./__pages_webmestre'))
	{
		...
		$DB_SQL = ($news_id) ? 'SELECT * FROM sacoche_rss WHERE sacoche_rss_id='.$news_id.' LIMIT 1' : 'SELECT * FROM sacoche_rss ORDER BY rss_date DESC LIMIT 1';
		$DB_ROW = DB::queryRow(SACOCHE_WEBMESTRE_BD_NAME , $DB_SQL);
		if(count($DB_ROW))
		{
			echo'<li><b>'.html($DB_ROW['rss_titre']).'</b> ['.convert_date_mysql_to_french($DB_ROW['rss_date']).']<br />'.str_replace(array("\r\n","\r","\n"),'<br />',html($DB_ROW['rss_contenu'])).'</li>';
		}
	}
	?>
