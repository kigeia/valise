<?php
/**
 * @version $Id: fichier_import-eleve-classe.ajax.php 8 2009-10-30 20:56:02Z thomas $
...
elseif( $step==2 )
{
	...
	file_put_contents($dossier_import.'import_'.$_SESSION['STRUCTURE_ID'].'_eleves.txt',serialize($tab_eleves_fichier));
	$tab_classes_fichier = array('ref'=>$tab_classes['ref']);
	file_put_contents($dossier_import.'import_'.$_SESSION['STRUCTURE_ID'].'_classes.txt',serialize($tab_classes_fichier));
	echo'<div id="ok">';
...
elseif( $step==3 )
{
	$fnom = 'import_'.$_SESSION['STRUCTURE_ID'].'_classes.txt';
...
elseif( $step==5 )
{
	$fnom = 'import_'.$_SESSION['STRUCTURE_ID'].'_eleves.txt';
	if(!file_exists($dossier_import.$fnom))
	{
	...
	foreach($DB_TAB as $key => $DB_ROW)
	{
		...
		$tab_eleves_base['nom'][$DB_ROW['livret_user_id']]        = $DB_ROW['livret_user_nom'];
		$tab_eleves_base['prenom'][$DB_ROW['livret_user_id']]     = $DB_ROW['livret_user_prenom'];
		$tab_eleves_base['classe'][$DB_ROW['livret_user_id']]     = $DB_ROW['livret_groupe_ref'];
		$tab_eleves_base['statut'][$DB_ROW['livret_user_id']]     = $DB_ROW['livret_user_statut'];
	}
	...
	file_put_contents($dossier_import.'import_'.$_SESSION['STRUCTURE_ID'].'_traitement_eleve.txt',serialize($tab_traitement));
	// On affiche
	echo'<div id="ok">';
...
elseif( $step==6 )
{
	$fnom = 'import_'.$_SESSION['STRUCTURE_ID'].'_traitement_eleve.txt';
	if(!file_exists($dossier_import.$fnom))
	{
	...
	if(count($tab_mod))
	{
		foreach($tab_mod as $id)
		{
			...
			foreach($tab_champs as $champ_ref)
			{
				...
				{
					$tab_set[] = ($champ_ref!='classe') ? 'livret_user_'.$champ_ref.'=:'.$champ_ref : 'livret_eleve_'.$champ_ref.'_id=:'.$champ_ref;
	...
	foreach($DB_TAB as $key => $DB_ROW)
	{
		...
		$lignes .= '<tr'.$class.'><td>'.html($DB_ROW['livret_user_num_sconet']).'</td><td>'.html($DB_ROW['livret_user_reference']).'</td><td>'.html($DB_ROW['livret_user_nom']).'</td><td>'.html($DB_ROW['livret_user_prenom']).'</td><td>'.html($DB_ROW['livret_groupe_ref']).'</td><td'.$class.'>'.html($DB_ROW['livret_user_login']).'</td>'.$td_password.'<td>'.$tab_statut[$DB_ROW['livret_user_statut']].'</td></tr>'."\r\n";
	}
