...
if( ($action=='prof_login') && $nb )
{
	...
	foreach($DB_TAB as $DB_ROW)
	{
		$fcontenu .= $DB_ROW['user_num_sconet']."\t".$DB_ROW['user_reference']."\t".$DB_ROW['user_profil']."\t".$DB_ROW['user_nom']."\t".$DB_ROW['user_prenom']."\t".$tab_login[$DB_ROW['user_id']]."\t".'inchangé'."\r\n";
	}
	$zip = new ZipArchive();
	...
	foreach($DB_TAB as $DB_ROW)
	{
		$text = mb_strtoupper($DB_ROW['user_profil'])."\r\n";
...
elseif( ($action=='prof_mdp') && $nb )
{
	...
	foreach($DB_TAB as $DB_ROW)
	{
		$fcontenu .= $DB_ROW['user_num_sconet']."\t".$DB_ROW['user_reference']."\t".$DB_ROW['user_profil']."\t".$DB_ROW['user_nom']."\t".$DB_ROW['user_prenom']."\t".$DB_ROW['user_login']."\t".$tab_password[$DB_ROW['user_id']]."\r\n";
	}
	$zip = new ZipArchive();
	if ($zip->open($dossier_login_mdp.$fnom.'.zip', ZIPARCHIVE::CREATE)===TRUE)
	{
