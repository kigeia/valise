...
if( ($action=='prof_login') && $nb )
{
	...
	{
		// Construire le login
		$login_prenom = mb_substr( clean_login($DB_ROW['livret_user_prenom']) , 0 , mb_substr_count($_SESSION['MODELE_PROF'],'p') );
		$login_nom    = mb_substr( clean_login($DB_ROW['livret_user_nom'])    , 0 , mb_substr_count($_SESSION['MODELE_PROF'],'n') );
