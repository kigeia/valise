...
if( (($action=='ajouter')||($action=='dupliquer')) && $ordre && $nom )
{
	...
	if( DB_tester_periode_nom($nom) )
	{
		exit('Erreur : nom de période déjà existant !');
	}
	// Insérer l'enregistrement
	$periode_id = DB_ajouter_periode($ordre,$nom);
	// Afficher le retour
...
{
	// Vérifier que le nom de la période est disponible
	if( DB_tester_periode_nom($nom,$id) )
	{
		exit('Erreur : nom de période déjà existant !');
	}
	// Mettre à jour l'enregistrement
	DB_modifier_periode($id,$ordre,$nom);
	// Afficher le retour
	echo'<td>'.$ordre.'</td>';
	echo'<td>'.html($nom).'</td>';
	echo'<td class="nu">';
