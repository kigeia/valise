<?php
/**
 * @version $Id: fichier_import-id-gepi.ajax.php 8 2009-10-30 20:56:02Z thomas $
...
{
	$DB_SQL = 'UPDATE livret_user ';
	$DB_SQL.= 'SET livret_user_id_gepi=livret_user_login ';
	$DB_SQL.= 'WHERE livret_structure_id=:structure_id ';
	$DB_VAR = array(':structure_id'=>$_SESSION['STRUCTURE_ID']);
	DB::query(SACOCHE_BD_NAME , $DB_SQL , $DB_VAR);
	exit('ok');
...
if($action=='import_gepi_profs')
{
	...
	$fichier_dest = 'import-gepi-profs_'.$_SESSION['STRUCTURE_ID'].'.txt' ;
			...
			else
			{
				...
				else
				{
					...
					else
					{
						...
						$DB_SQL = 'UPDATE livret_user ';
						$DB_SQL.= 'SET livret_user_id_gepi=:id_gepi ';
						$DB_SQL.= 'WHERE livret_structure_id=:structure_id AND livret_user_id=:user_id ';
						$DB_SQL.= 'LIMIT 1';
						$DB_VAR = array(':structure_id'=>$_SESSION['STRUCTURE_ID'],':user_id'=>$id,':id_gepi'=>$id_gepi);
						DB::query(SACOCHE_BD_NAME , $DB_SQL , $DB_VAR);
...
if($action=='import_gepi_eleves')
{
	...
	$fichier_dest = 'import-gepi-eleves_'.$_SESSION['STRUCTURE_ID'].'.txt' ;
	if(!move_uploaded_file($fnom_serveur , $dossier_import.$fichier_dest))
	{
		exit('Erreur : le fichier n\'a pas pu être enregistré sur le serveur.');
	}
	...
	foreach($DB_TAB as $DB_ROW)
	{
		...
		$tab_users_base['nom'][$DB_ROW['livret_user_id']]        = $DB_ROW['livret_user_nom'];
		$tab_users_base['prenom'][$DB_ROW['livret_user_id']]     = $DB_ROW['livret_user_prenom'];
		$tab_users_base['num_sconet'][$DB_ROW['livret_user_id']] = $DB_ROW['livret_user_num_sconet'];
	}
	// Observer le contenu du fichier et comparer avec le contenu de la base
	$lignes_ras = '';
	$lignes_mod = '';
