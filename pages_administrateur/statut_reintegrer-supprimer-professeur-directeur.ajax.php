<?php
/**
 * @version $Id: statut_reintegrer-supprimer-professeur-directeur.ajax.php 8 2009-10-30 20:56:02Z thomas $
...
elseif( ($action=='supprimer') && $nb )
{
	foreach($tab_select_users as $user_id)
	{
		...
		$DB_VAR = array(':structure_id'=>$_SESSION['STRUCTURE_ID'],':user_id'=>$user_id);
		DB::query(SACOCHE_BD_NAME , $DB_SQL , $DB_VAR);
		$DB_SQL = 'DELETE livret_jointure_evaluation_competence FROM livret_jointure_evaluation_competence ';
		...
		$DB_SQL.= 'LEFT JOIN livret_evaluation ON livret_groupe.livret_groupe_prof_id=livret_evaluation.livret_prof_id ';
